...
from panoroma_encoder.equiconv_encoder import PanoramaVisionEncoder
...
logger = Logger('train_panorama_crossarea.log')
...
def evaluate_lora(clip_model, vision_encoder,image_projector, loader, logit_scale, visualize=False, num_visualizations=5, iter=0):
    vision_encoder.eval()
...
class ImageProjection(nn.Module):
    ...
    def forward(self, image_embeddings):
        projected_embeddings = self.image_projection(image_embeddings.squeeze())
...
# Panoarama Encoder
layerdict = torch.load('/home/erzurumlu.1/yunus/git/whereami/checkpoints/panorama_unet/offset_layer/layer_256x512-b128.pt')
offsetdict = torch.load('/home/erzurumlu.1/yunus/git/whereami/checkpoints/panorama_unet/offset_layer/offset_256x512-b128.pt')
vision_encoder = PanoramaVisionEncoder(layer_dict=layerdict, offset_dict=offsetdict)
vision_encoder.load_state_dict(torch.load('/home/erzurumlu.1/yunus/git/whereami/checkpoints/panorama_unet/equi_panorama_encoder.pth'))
vision_encoder.to(device)
# Image Projection
shared_embedding_size = 512
image_projection = ImageProjection(2048, shared_embedding_size)
...
load_path = '/home/erzurumlu.1/yunus/git/CLIP-LoRA/vitb16/1/lora_weights_27750.pt'
clip_model = clip_model.to(device)
load_lora(args,load_path, list_lora_layers)
...
train_dataset = VigorPlus('/home/erzurumlu.1/yunus/research_drive/data/VIGOR', ['NewYork', 'Seattle'], transform=train_transform, is_satellite=False)
val_dataset = VigorPlus('/home/erzurumlu.1/yunus/research_drive/data/VIGOR', ['Chicago', 'SanFrancisco'], transform=val_transform, val=True, is_satellite=False)
...
while count_iters < total_iters:
    ...
    for images, captions in train_loader:
        images = images.to(device) # Shape: [batch_size, 3, 256, 512]
        texts = clip.tokenize(captions, truncate=True).to(device) # [B. 77]
        optimizer.zero_grad()
        with torch.cuda.amp.autocast():
            image_features = vision_encoder(images) # [B, 2048, 1, 1]
            text_features = clip_model.encode_text(texts) # [B, 512]
            image_features = image_projection(image_features) # [B, 512]
            loss = clip_loss(text_features, image_features, logit_scale)
            total_loss += loss.item()

            if count_iters  == 0:
                evaluate_lora(clip_model, vision_encoder, image_projection, val_loader,logit_scale)
                evaluate_lora(clip_model, vision_encoder, image_projection, val_loader,logit_scale, visualize=True, iter=count_iters)
            ...
            if count_iters % 500 == 0:
                evaluate_lora(args, clip_model, vision_encoder, image_projection, val_loader,logit_scale)
                vision_encoder.train()
                image_projection.train()
                
                # Save the model
                torch.save(vision_encoder.state_dict(), f'/home/erzurumlu.1/yunus/git/whereami/checkpoints/panorama_unet/language_training/panorama_vision_encoder_{count_iters}.pth')
                torch.save(image_projection.state_dict(), f'/home/erzurumlu.1/yunus/git/whereami/checkpoints/panorama_unet/language_training/image_projection_{count_iters}.pth')
                logger.info("**** saved encoder weights ****")

            if count_iters % 1000 == 0:
                evaluate_lora(args, clip_model, vision_encoder, image_projection, val_loader,logit_scale, visualize=True, iter=count_iters)
                vision_encoder.train()
                image_projection.train()
